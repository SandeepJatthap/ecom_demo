...
class CartBloc extends Bloc<CartEvent, CartState> {
  ...
  CartBloc(
    ...
    on<RemoveProduct>(_onRemoveToCart);
  ...
  void _onRemoveToCart(RemoveProduct event, Emitter<CartState> emit) async {
    try {
      emit(CartLoading(cart: state.cart));
      List<CartItem> cart = state.cart;

      var result = await _removeCartUseCase(event.cartItem);
      result.fold(
        (failure) => emit(CartError(cart: state.cart, failure: failure)),
        (_) {
          cart.removeWhere(
              (element) => element.product.id == event.cartItem.product.id);
