    ...
    try {
      ...
      result.fold(
        ...
        (productResponse) {
          ...
          if (event.params.keyword != null &&
              event.params.keyword!.isNotEmpty) {
            for (var action in productResponse) {
              if (action.title!
                  .toLowerCase()
                  .contains(event.params.keyword!.trim())) {
                finalList.add(action);
              }
            }
          } else {
            finalList = productResponse;
          }
          emit(ProductLoaded(
            metaData: PaginationMetaData(
                limit: productResponse.length,
